...
Finally, scan for **depth gaps** — signals that code-level coverage exists but human context is needed:
- **Lifecycle without checkpoints:** PROC- artifacts that describe state transitions but have no Key Facts about what validation runs at each boundary. These are candidates for `/reef:deep` where the user can explain what *should* happen at each step.
- **Contracts without endpoint detail:** CON- artifacts that say "Service A calls Service B" but list no specific endpoints, call order, or error handling. Flag for interactive deepening.
- **Single-app coverage of multi-app entities:** If `project.json` lists sub_apps for a service but PROC-/SCH- artifacts only reference one app, flag: "Does {other_app} handle this the same way?"
- **Schema fields with no runtime reference:** Fields that exist in SCH- artifacts but appear in no PROC- or API- artifact's Key Facts. May be governance metadata that's modeled but not enforced.
- **Decision-shaped code without rationale:** Observable patterns that look like deliberate choices (e.g., two auth mechanisms coexisting, different versioning strategies per entity) but have no DEC- artifact explaining why. Flag as "observable decision — was this intentional?"

Surface these depth gaps alongside the repetition gaps in the numbered list. Frame each as a question, not a missing artifact type.
...
6. **Refresh CLAUDE.md** at the reef root. Update artifact counts, CON- list, SYS- list, RISK- list, key known_unknowns, and freshness date. Use the template in `${CLAUDE_PLUGIN_ROOT}/references/claude-md-template.md`.

7. **Render health report** using the same Unicode box-drawing format as snorkel Step 7. This gives the user an immediate visual sense of how much richer the reef got from Phase 1.
...
10. **After answering questions,** if the user learned something new or wants to explore a tangent that's outside the current question agenda, suggest: "Want to explore that further? `/reef:artifact` can dig into a specific topic or capture what you just learned."
