...
- If one `.reef/` is found: read its `project.json` and check the reef's current state:
  ...
  **State detection — check in this order (first match wins):**

  1. Check `.reef/scuba-manifest.json`: read it, look at `planned` and `completed` arrays.
     - If it exists AND `planned` is empty AND `completed` is non-empty → **scuba complete.** Recommend: "Reef is built. Next steps: `/reef:update` to sync with recent code changes, `/reef:deep` to trace critical areas line-by-line, or `/reef:health` for a status check"
     - If it exists AND `planned` is non-empty → **scuba in progress.** Recommend: "Run `/reef:scuba` to continue (N items remaining)"
  2. Count artifacts. Count specs in `sources/apis/` and `sources/schemas/`.
     - 0 artifacts AND 0 specs → "Run `/reef:snorkel` and `/reef:source` to start discovery"
     - Artifacts exist but 0 specs → "Run `/reef:source` to extract API specs and ERDs"
     - Artifacts + specs exist but no scuba manifest → "Run `/reef:scuba` to deepen artifacts with domain knowledge"
...
If yes: run `git -C <repo-path> pull` for each selected repo. Report results briefly ("3/3 up to date" or "repo-a: 5 commits pulled, repo-b: up to date").
...
- **Other failure** (uncommitted changes, merge conflicts): warn and continue with the stale copy.
...
4. **Generate CLAUDE.md** at the reef root. This makes the reef discoverable by AI agents. Use the template in `${CLAUDE_PLUGIN_ROOT}/references/claude-md-template.md`. At this stage, populate:
   - Project name and description (from `project.json`)
   - Services list with descriptions and source repos
   - Artifact counts by type (from what snorkel just created)
   - CON- artifacts listed prominently (from `artifacts/contracts/`)
   - SYS- artifacts listed (from `artifacts/systems/`)
   - Freshness date = now

   This is the initial version. Snorkel, scuba, update, and deep will refresh it as the reef grows.

5. **Suggest next steps:** `/reef:scuba` to deepen artifacts, `/reef:test` to check coverage. Mention that artifacts are wikilinked and can be opened as an Obsidian vault for graph visualization.
