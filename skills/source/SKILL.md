...
For each repo, scan for infrastructure signals that define how data flows *outside* of databases and APIs — cloud storage, message queues, scheduled jobs, deployment topology, and configuration surfaces. This is especially important for pipeline/orchestration services that have few or no database entities.

**Read `references/tech-stack-signals.md`** — the Infrastructure / Storage Indicators section — for detection signals.

**Skip conditions:** If a service already has rich SCH- and API- extractions (10+ entities, 20+ endpoints), infrastructure extraction is lower priority — the service's complexity is well-captured by traditional extraction. Focus infrastructure extraction on services where API + ERD extraction produced thin results.

### 3.5.1 — Detect infrastructure signals
...
### 3.5.2 — Write infrastructure files

Output to `sources/infra/{service}/`:

**`storage.md`** — when cloud storage patterns are detected:
...
**`runtime.md`** — when Docker/K8s configs are detected:
...
**`queues.md`** — when message queue patterns are detected:
...
**If no infrastructure signals are found for a service, skip it.** Do not create empty files.
