...
#### 4a. Artifact topology

Show a heatmap matrix of artifact density by service and type. This is a factual summary derived from frontmatter — no inference about system architecture, no dependency arrows, no guessing at relationships. The user sees the shape of their reef and notices the gaps themselves.
...
The goal is to let the user *see* where coverage is dense vs thin at a glance. Empty cells and light cells next to dark cells naturally invite the question "why?"

**Rules:**
- One table with legend, scannable in 5 seconds. No diagrams, no arrows, no architecture inference.
- Counts come from artifact frontmatter `type` and `domain` fields.
- Do not editorialize about gaps — let the heatmap speak. The user will notice.
...
From the reef reading in Step 3, identify areas where surface-level coverage hides real complexity. Use these structural signals to generate curiosity hooks:

**From existing artifact signals:**
...
**From gap patterns (these surface knowledge that only the user can provide):**
- **Lifecycle without checkpoints.** A PROC- artifact describes states (draft → active → finalized) but has no Key Facts about what validation runs at each transition. Ask: "What actually happens when X is finalized — what gets checked, what side effects fire?"
- **Cross-service contract without endpoint detail.** A CON- artifact says "Service A calls Service B" but lists no specific endpoints, call order, or failure behavior. Ask: "What exactly happens at this boundary, and what breaks if one side changes?"
- **Single-app coverage of multi-app entity.** SCH-/PROC- artifacts cover one app (e.g., US region) but the service has parallel apps (US + EU). Ask: "Does the other app handle this the same way, or are there differences that matter?"
- **Code-observable pattern without rationale.** A DEC- or PAT- artifact describes what the code does but not why — or an observable decision exists with no DEC- artifact at all. Ask: "Why was it built this way? Was there an incident or alternative that was rejected?"
- **Modeled but not enforced.** Schema fields that look like governance controls (allowed_usages, retention_days, must_destroy) exist but no PROC- artifact references them being checked at runtime. Ask: "Are these actually enforced, or just metadata? Is enforcement planned?"
...
If the dive surfaced topics the user wants to return to later or knowledge worth capturing outside the current thread, suggest: "You can also use `/reef:artifact` anytime to explore a topic, capture something you learned, or refine a specific artifact."
