...
def cmd_detect_new(args) -> None:
    """Detect new source files added since last update and check artifact coverage."""
    import subprocess

    reef = find_reef_root(args.reef)
    project_path = reef / ".reef" / "project.json"
    if not project_path.is_file():
        fail("No project.json found")

    project = read_json(project_path)

    # Collect all artifact body text for searching
    artifacts = collect_artifacts(reef)
    artifact_text: dict[str, str] = {}  # id -> full body text
    for apath, fm in artifacts:
        aid = fm.get("id", "")
        try:
            artifact_text[aid] = apath.read_text(encoding="utf-8").lower()
        except Exception:
            artifact_text[aid] = ""

    # Model file patterns
    MODEL_PATTERNS = [
        r"*/models/*.py", r"*/models.py", r"*/entities/*.ts",
        r"*/entity/*.ts", r"*/models/*.go", r"*/model/*.go",
        r"*/models/*.java", r"*/models/*.rs",
    ]
    MODEL_DIR_NAMES = {"models", "entities", "entity", "model"}

    def is_model_file(filepath: str) -> bool:
        parts = Path(filepath).parts
        return any(p in MODEL_DIR_NAMES for p in parts) or filepath.endswith("models.py")

    def is_api_file(filepath: str) -> bool:
        parts = Path(filepath).parts
        api_dirs = {"endpoints", "routes", "controllers", "handlers", "views", "api", "routers"}
        return any(p in api_dirs for p in parts)

    def is_skip_file(filepath: str) -> bool:
        parts = Path(filepath).parts
        skip_dirs = {"tests", "test", "migrations", "alembic", "__pycache__", "node_modules"}
        skip_suffixes = ("_test.py", "_test.go", "_test.ts", ".test.ts", ".test.js", ".spec.ts")
        return (
            any(p in skip_dirs for p in parts)
            or filepath.endswith(skip_suffixes)
            or "__init__" in filepath
        )

    def extract_class_names(filepath: Path) -> list[str]:
        """Extract class names from a source file."""
        try:
            text = filepath.read_text(encoding="utf-8")
        except Exception:
            return []
        # Python: class FooBar(Base):
        classes = re.findall(r"^class\s+(\w+)\s*[:(]", text, re.MULTILINE)
        # TypeScript: export class FooBar
        classes += re.findall(r"export\s+class\s+(\w+)", text)
        # TypeORM: @Entity()
        if "@Entity" in text:
            classes += re.findall(r"export\s+class\s+(\w+)", text)
        return [c for c in classes if not c.startswith("_") and c not in ("Base", "Model", "Meta")]

    def find_covering_artifact(class_name: str) -> str | None:
        """Check if any artifact mentions this class name."""
        cn_lower = class_name.lower()
        # Also try snake_case version
        snake = re.sub(r"(?<=[a-z])(?=[A-Z])", "_", class_name).lower()
        for aid, text in artifact_text.items():
            if cn_lower in text or snake in text:
                return aid
        return None
    ...
    for source in project.get("sources", []):
        ...
        # Find new files since the last source index was built
        new_files = []
        try:
            # Use source-index timestamp to find commits added since last indexing
            source_index_path = reef / ".reef" / "source-index.json"
            since_date = None
            if source_index_path.is_file():
                si = read_json(source_index_path)
                since_date = si.get("indexed_at")

            if since_date:
                # Find files added in commits after the index timestamp
                result = subprocess.run(
                    ["git", "-C", str(repo_path), "log", "--diff-filter=A",
                     "--name-only", "--format=", f"--since={since_date}"],
                    capture_output=True, text=True, timeout=10
                )
            else:
                # No index timestamp — compare against last 30 days as fallback
                result = subprocess.run(
                    ["git", "-C", str(repo_path), "log", "--diff-filter=A",
                     "--name-only", "--format=", "--since=30 days ago"],
                    capture_output=True, text=True, timeout=10
                )
            if result.returncode == 0:
                ...
                # Deduplicate (a file may appear in multiple commits)
                new_files = list(dict.fromkeys(new_files))
        except Exception:
            # Fallback: no git info available
            pass
        ...
        for filepath in new_files:
            ...
            if is_model_file(filepath):
                entry["type"] = "model"
                classes = extract_class_names(full_path)
                entry["classes"] = classes
                uncovered = []
                covering = None
                for cls in classes:
                    art = find_covering_artifact(cls)
                    if art:
                        covering = art
                    else:
                        uncovered.append(cls)
                entry["covered_by"] = covering
                entry["uncovered_classes"] = uncovered
            elif is_api_file(filepath):
                entry["type"] = "api"

            results.append(entry)

    # Detect cross-app models: same class name appearing in multiple repos/apps
    # These are cross-cutting concerns that warrant their own artifact, not enrichment
    from collections import defaultdict
    class_to_repos: dict[str, list[str]] = defaultdict(list)
    for r in results:
        if r["type"] == "model":
            for cls in r["classes"]:
                class_to_repos[cls].append(r["repo"])

    cross_app_classes = {cls for cls, repos in class_to_repos.items() if len(set(repos)) > 1}

    # Summarize
    model_files = [r for r in results if r["type"] == "model"]
    api_files = [r for r in results if r["type"] == "api"]
    uncovered_entities = []
    for r in model_files:
        for cls in r["uncovered_classes"]:
            uncovered_entities.append({
                "class": cls,
                "file": f"{r['repo']}/{r['file']}",
                "nearest_artifact": r["covered_by"],
                "cross_app": cls in cross_app_classes,
            })

    emit({
        "status": "ok",
        "new_files_total": len(results),
        "new_model_files": len(model_files),
        "new_api_files": len(api_files),
        "uncovered_entities": uncovered_entities,
        "cross_app_classes": sorted(cross_app_classes),
        "files": results,
    })


def cmd_index_context(args) -> None:
    reef = find_reef_root(args.reef)
    context_dir = reef / "sources" / "context"
    raw_dir = reef / "sources" / "raw"

    # Load previous index if it exists
    index_path = reef / ".reef" / "context-index.json"
    prev_index: dict[str, dict] = {}
    if index_path.is_file():
        prev_data = read_json(index_path)
        prev_index = prev_data.get("files", {})

    # Collect all artifact source references for "referenced_by"
    artifact_refs: dict[str, list[str]] = {}
    for art_path, fm in collect_artifacts(reef):
        art_id = fm.get("id", art_path.stem)
        for src in fm.get("sources", []) or []:
            src_str = str(src)
            artifact_refs.setdefault(src_str, []).append(art_id)

    # Walk context and raw dirs
    files_map: dict[str, dict] = {}
    scan_dirs = []
    if context_dir.is_dir():
        scan_dirs.append(context_dir)
    if raw_dir.is_dir():
    ...
    for scan_root in scan_dirs:
        for dirpath, _dirnames, filenames in os.walk(scan_root):
            for fname in filenames:
                fpath = Path(dirpath) / fname
                try:
                    rel = str(fpath.relative_to(reef / "sources"))
                    stat = fpath.stat()
                    h = sha256_file(fpath) if not is_binary(fpath) else None

                    # Find which artifacts reference this file
                    refs = []
                    for ref_key, art_ids in artifact_refs.items():
                        if rel in ref_key or fname in ref_key:
                            refs.extend(art_ids)
    ...
    new_files = [f for f in files_map if f not in prev_index]
    changed_files = [
        f for f in files_map
        if f in prev_index and files_map[f]["hash"] != prev_index[f].get("hash")
    ]
    removed_files = [f for f in prev_index if f not in files_map]
    unreferenced = [f for f, v in files_map.items() if not v["referenced_by"]]

    # Write index
    index_data = {"indexed_at": now_iso(), "files": files_map}
...
def main() -> None:
    ...
    # detect-new
    p_detect = sub.add_parser("detect-new", help="Detect new source files and check artifact coverage")
    p_detect.add_argument("--reef", default=None, help="Path to reef root")
    p_detect.set_defaults(func=cmd_detect_new)
